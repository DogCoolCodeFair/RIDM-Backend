...
try:
    ...
    for idx in range(1, 114):
        ...
        for i in tbody.find_all("tr"):
            code = str(i.find_all("td")[1].find("a")).replace('''return false;" title="질환정보 바로가기"><span>질환정보 바로가기</span></a>''', "").replace('''<a href="#" onclick="javascript:fn_moveDetail(''', "").replace("');","").replace("'","")
            ...
            name = str(i.find("td").find("dl").find("dt")).replace("<dt>","").replace("</p></dt>","").split("<p>")
            krname = name[0]
            engname = name[1]
            ...
            benefit_code = str(i.find("td").find("dl").find_all("li")[4]).replace('''<li style="padding-left: 20px !important"><span>산정특례 특정기호 :</span> ''', '''''').replace("</li>","")
            kcd_code = str(i.find("td").find("dl").find_all("li")[3]).replace('''<li><span>KCD코드 :</span> ''',"").replace("</li>","")
            ...
            affected = tbody2.find_all("tr")[0].find_all("td")[1].text.replace("체내 : ","").replace("체외 : ",",").strip().split(",")
